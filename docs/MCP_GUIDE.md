...
内联工具（`deploy_page` / `deploy_files` / `deploy_markdown` / `deploy_docs`）把内容当作工具参数传入——**单文件超过 10 MB 会被拒绝**，且大文件会让模型逐 token 生成超长参数，又慢又容易卡死。

用 **方式三 的 npm 连接器**（`pagefire-mcp`）时，会额外多出三个**本地文件**工具：它们读取你磁盘上的文件、**带外直传**到服务器（`/upload`），内容完全不经过模型，因此文件大小/数量不再受"工具参数"瓶颈限制。

| 工具 | 作用 | 关键参数 |
|------|------|----------|
| `deploy_dir` | 把本地目录递归发布为静态站点（须含 `index.html`） | `dir`、`spa`、`did`、`pin`… |
| `deploy_docs_dir` | 读取目录下所有 `.md`，渲染成带侧边栏的文档站 | `dir`、`theme`、`did`、`pin`… |
| `deploy_file` | 发布单个本地文件（HTML 作为入口） | `path`、`did`、`pin`… |

**对话示例：**

```
把 ./dist 目录发布成网站，开启 SPA 模式，永久保留。
把 docs/ 目录里的所有 markdown 发布成文档站，用 dark 主题。
```

> 这些工具只在 npm 连接器（方式三）下可用；HTTP 直连（方式一）只暴露服务端的内联工具。单文件仍限 10 MB，但目录可包含很多文件、总量受 token 配额（200 MB）约束。
